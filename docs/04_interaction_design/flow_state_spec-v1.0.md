...
stateDiagram-v2
    [*] --> INIT : 发起创建
    INIT --> ACTIVE : 拆解就绪或对话确认Task
    ACTIVE --> SUSPENDED : 24小时无交互
    SUSPENDED --> ACTIVE : 一键重载唤醒
...
| **INIT**      | **ACTIVE**    | 阅读: 自动拆解 / 计划: 对话确认 | **阅读项目**：文件切片与大纲解析完成，系统自动按章节拆解生成 Task Chain (`READING_CHAPTER`) 及默认 Task，绑定伴读 Agent 后状态**自动转为 `ACTIVE`**。<br>**计划项目**：载入 Skill 模板后生成预选建议，用户在工作台与监督 Agent 对话沟通并确认要创建的 Task 任务树后，后端将状态由 `INIT` 扭转为 `ACTIVE`。 |
