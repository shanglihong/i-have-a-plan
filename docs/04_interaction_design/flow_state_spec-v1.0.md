...
### 5. 阅读笔记实体 (Reading Note) 状态机

阅读笔记承载用户的主观思考、划词高亮片段及伴读精华问答，其编辑权限随父级项目状态强级联。

```mermaid
stateDiagram-v2
    [*] --> EDITABLE : 创建笔记 (划词/对话转存/手写)
    EDITABLE --> ARCHIVED : 项目归档时级联触发 (只读锁定)
    ARCHIVED --> EDITABLE : 项目被重新激活时恢复
    EDITABLE --> [*] : 手动删除笔记
```

| 源状态 | 目标状态 | 触发事件 | 前端交互与行为契约 |
| :--- | :--- | :--- | :--- |
| **[*]** | **EDITABLE** | 划词写笔记 / 对话一键存笔记 | 前端笔记面板自动展开，滑出插入笔记卡片并聚焦输入框。后端生成笔记实体并与原文物理锚点（source_anchor）绑定。 |
| **EDITABLE** | **ARCHIVED** | 项目标记归档 | 后端拦截修改/删除 API。前端笔记面板整体呈置灰强只读模式，禁用任何交互输入组件。 |
| **ARCHIVED** | **EDITABLE** | 项目重新激活 | 前端面板淡出只读提示，恢复可编辑态。后端解封修改与删除 API。 |
...
展示用户在沉浸式阅读时，系统如何通过章节末 5% 的推荐气泡引导用户进行启发式伴读与知识内化的交互时序。
...
sequenceDiagram
    ...
    participant FE_Discuss as 前端右侧Discuss伴读
    participant BE as 后端伴读服务
    participant DB as 数据库/Redis

    %% --- 场景 A: 沉浸式阅读与气泡触发 ---
    Note over User, FE_Reader: 沉浸式阅读阶段 (AI 伴读静默)
    User->>FE_Reader: 持续向下滚动阅读
    FE_Reader->>FE_Reader: 监测滚动条高度
    alt 达到章节倒数 5% 临界点 (例如 95% 滚动高度)
        FE_Reader->>FE_Bubble: 发送气泡展示事件
        FE_Bubble->>FE_Bubble: 以淡入微动效滑出弱提示气泡
    end

    %% --- 场景 B: 忽略气泡继续阅读 ---
    alt 场景 B1: 用户选择忽略气泡并继续阅读
        User->>FE_Reader: 继续滚动跨越当前章节边界
        FE_Reader->>FE_Bubble: 发送跨章事件
        FE_Bubble->>FE_Bubble: 气泡平滑淡出并销毁
    else 场景 B2: 用户点击气泡开启伴读
        User->>FE_Bubble: 点击弱提示气泡
        FE_Bubble->>FE_Discuss: 广播激活伴读请求
        FE_Bubble->>FE_Bubble: 气泡淡出消失
        FE_Discuss->>FE_Discuss: 自动滑出并聚焦伴读面板
        FE_Discuss->>BE: 请求本章启发式伴读内容 (携带项目ID与章节ID)
        BE->>DB: 读取章节大纲与 AI 预先生成的启发提问
        DB-->>BE: 返回本章小结与思考题
        BE-->>FE_Discuss: 渲染 AI 导师的启发式伴读引导消息
        
        %% --- 知识内化与任务引导 ---
        Note over User, FE_Discuss: 知识内化与章节任务引导
        User->>FE_Discuss: 与 AI 导师进行伴读讨论/回答问题
        FE_Discuss->>BE: 发送讨论文本
        BE-->>FE_Discuss: 返回启发式反馈
        Note over BE: 判定探讨已覆盖核心方法论
        BE-->>FE_Discuss: 触发“章节末任务”生成建议 (如：记录实践总结)
        User->>FE_Discuss: 点击“一键加入计划”
        FE_Discuss->>BE: 确认生成章节任务请求
        BE->>DB: 创建并关联新 Task (PENDING 状态)，并更新大纲章节为“已读/已讨论”
        DB-->>BE: 保存成功
        BE-->>FE_Reader: 广播进度更新事件 (触发双维度加权进度重新计算)
        FE_Reader->>FE_Reader: 重新渲染进度条与章节已读状态
    end
```
...
### 5. 双通道笔记沉淀与 Trace-to-Source 定位时序

展示用户通过“原文划词”及“伴读对话转存”双通道沉淀阅读笔记的过程，以及通过点击笔记卡片反向精准定位原文的双向跳转交互。
...
sequenceDiagram
    ...
    actor User as 用户
    participant FE_Reader as 前端左侧阅读器
    participant FE_Note as 前端中间笔记面板
    participant FE_Discuss as 前端右侧Discuss伴读
    participant BE as 后端服务
    participant DB as 数据库

    %% --- 场景 A: 划词高亮写笔记 ---
    Note over User, FE_Note: 场景 A: 划词高亮写笔记
    User->>FE_Reader: 划选段落文本
    FE_Reader->>FE_Reader: 弹出浮动菜单
    User->>FE_Reader: 点击“高亮并记笔记”
    FE_Reader->>FE_Reader: 原文设为淡绿色高亮
    FE_Reader->>FE_Note: 发送创建事件 (携带高亮内容与相对锚点)
    FE_Note->>FE_Note: 笔记抽屉自动滑出展开<br>在当前章节下淡入生成一条笔记卡片
    FE_Note->>FE_Note: 自动插入引用代码块，光标聚焦输入框
    User->>FE_Note: 输入个人想法感悟并保存
    FE_Note->>BE: 提交笔记数据 (内容 + 原文物理锚点)
    BE->>DB: 存储阅读笔记实体
    DB-->>FE_Note: 返回保存成功

    %% --- 场景 B: 对话一键转存笔记与 Trace-to-Source 定位 ---
    Note over User, FE_Discuss: 场景 B: 对话一键转存笔记与 Trace-to-Source 双向跳转
    User->>FE_Discuss: 鼠标悬停在某条对话气泡上
    FE_Discuss->>FE_Discuss: 浮现“存为笔记”快捷图标
    User->>FE_Discuss: 点击“存为笔记”图标
    FE_Discuss->>FE_Note: 触发抛物线光流动画，发送对话上下文
    FE_Note->>FE_Note: 笔记抽屉滑出，在列表自动淡入插入富文本笔记卡片
    FE_Note->>BE: 提交笔记数据
    BE->>DB: 存储阅读笔记实体
    DB-->>FE_Note: 保存成功
    
    User->>FE_Note: 点击笔记卡片上的“原文引用链接”
    FE_Note->>FE_Reader: 广播跳转事件 (携带页码和垂直偏移锚点)
    FE_Reader->>FE_Reader: 平滑滚动 (Smooth Scroll) 定位到目标页码
    FE_Reader->>FE_Reader: 对被引用的段落触发 3 次脉冲闪烁高亮
```

---

### 6. Graph RAG 图谱闲时同步与可视化联动时序
...
* **三栏联动工作区布局**：
  - 界面打破传统双栏，采用经典的**左、中、右三栏联动布局**。
  - **左栏**：**阅读与大纲区**。顶部为项目标题及总体阅读进度条，主体为级联大纲树与 PDF/Markdown 阅读容器。
  - **中栏**：**笔记面板 (Note Panel)**。承载当前项目的阅读笔记卡片列表，包含划词引用块与富文本输入区。支持通过顶部或侧边手柄进行独立折叠与宽度拉伸。
  - **右栏**：**伴读交互区 (Discuss Area)**。承载与伴读 Agent 的对话聊天窗口。
* **总体阅读进度条更新与联动逻辑**：
  - **进度条计算规则**：
