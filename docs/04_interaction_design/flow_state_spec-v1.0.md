...
| **ACTIVE** | **SUSPENDED** | 24小时无交互 | 后端定时任务检测到会话超时（PA-04 契约），断开 LLM 活跃连接，将当前上下文及任务 Trace 持久化至 Redis，状态扭转为 `SUSPENDED`。 |
| **SUSPENDED** | **ACTIVE** | 用户重新登入并激活 | 前端弹出毛玻璃提示，用户点击“一键唤醒”，后端从 Redis 读取状态重建 LLM 会话，状态恢复为 `ACTIVE`。 |
...
* **超时休眠 (24小时)**：
  - 当用户在项目中持续 24 小时没有任何交互（包括伴读对话、任务状态标记、页面操作等），后端会将该项目的 LLM 会话（包含上下文、对话 Trace 栈、临时变量）序列化持久化存入 Redis 缓存，然后优雅释放服务器活跃会话通道，项目状态扭转为 `SUSPENDED`。
* **重载唤醒交互**：
  - 当休眠后用户重新打开该项目页面时：
    ...
    4. **后端重建**：后端在收到请求后，从 Redis 中读取持久化的上下文状态，在 500ms 内快速重建大模型会话上下文，用户可无缝继续之前的对话，没有任何历史丢失感。
...
| **PA-04** | **优雅休眠与重载契约** | 24小时无交互自动释放会话，上下文及 Trace 持久化到 Redis。重登时展示毛玻璃提示与“一键重载”按钮，点击后通过水波纹动效重载会话。 | 释放服务器 LLM 活跃连接句柄，防范连接死锁与资源溢出。 |
