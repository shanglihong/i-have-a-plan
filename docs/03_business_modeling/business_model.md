...
### 6. 碎片化思考笔记与原文物理上下文的割裂
* **业务痛点**：用户在阅读时记录的瞬时灵感或 Discuss 对话极其零碎，传统的笔记工具面临“知易行难”的物理隔离，即在回顾笔记时，无法快速、精准地反向跳回并高亮定位笔记所发生时的物理原文段落，导致“Trace-to-Source”信息回溯成本极高。
* **建模要求**：系统需引入“阅读笔记 (Reading Note)”实体，支持“划词写笔记”与“对话一键转笔记”双通道低成本捕获，并对笔记记录绑定的原文绝对锚点进行物理归档，支持 Smooth Scroll 原文并伴随高亮脉冲闪烁的联动机制。
...
| **思考灵感极简沉淀与追溯** | 在阅读中产生的感悟或讨论出的精华结论能低成本记录，并能在回顾笔记时一眼定位原文上下文细节。 | * 三栏式联动工作空间（大纲阅读 - 笔记卡片 - 伴读 Discuss）<br>* 划词高亮记笔记与对话一键转存双通道<br>* Trace-to-Source 双向跳转脉冲高亮动效 |
| **方法论技能可靠沉淀** | 能将单点笔记、章节精华或全书方法论，通过宏观打包 100% 正确地转化为可重复利用的技能模版。 | * Trace-to-Skill 三级提炼漏斗<br>* `SKILL.md` (YAML + Markdown 步骤) 规范<br>* 物理隔离的沙箱卡片编辑器 |
| **知行统一 of 计划指导** | 在启动新的实践项目时，无需从零拆解任务，可一键导入自己沉淀或系统推荐的 Skill 模版，生成实操任务链。 | * 普通计划项目初始化引导<br>* 语义检索匹配 Skill 并一键载入<br>* 任务树骨架屏渐进式分层渲染 |
...
### 6. 阅读笔记实体 (Reading Note)
* **定义**：用户在阅读文献或伴读讨论时，极低成本沉淀的碎片化感悟、高亮引用或 Discuss 对话记录。
* **核心属性**：`id`, `project_id`, `task_id` (关联的章节任务 ID), `source_type` (USER_HIGHLIGHT / AGENT_DISCUSS / FREE_WRITE), `content` (富文本/Markdown 笔记内容), `source_anchor` (绑定的物理原文页码、偏移量及引用代码块)。
* **业务规则**：
  * **物理追溯 (Trace-to-Source)**：对于通过原文划词（USER_HIGHLIGHT）及对话转存（AGENT_DISCUSS）生成的笔记，必须强制记录物理源文件的相对路径和精确偏移字符锚点。前台展示时，点击笔记上的“原文引用”，阅读容器必须平滑滚动并对引用行进行 3 次脉冲闪烁高亮，以追溯上下文细节。
  * **只读级联归档**：阅读笔记的编辑权限完全级联继承于关联项目。项目状态更新为 `ARCHIVED` 时，其下属的所有笔记同步转为强只读状态（只读横幅展示，禁用保存 API）。
  * **图谱抽取核心源**：在后台异步合并构建 Graph RAG 图数据库关系网时，笔记的内容必须与对应原文 Chunk 并行输入大模型，作为提炼实体与关系的重要事实源。
