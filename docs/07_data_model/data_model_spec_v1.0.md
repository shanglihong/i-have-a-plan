...
#### 2.1 UnifiedReadingNote（融合阅读笔记 - 聚合根）
**定义**：结合了用户主观感悟与 AI 伴读对话的综合卡片。与 Task 解耦，直接挂载于 Project 目录下。
...
#### 2.2 ExperienceNote（经验笔记 - 聚合根）
**定义**：项目完结时复盘产生的实战经验，驱动知识进化。
...
#### 3.1 Skill（技能模版 - 聚合根）
**定义**：由非结构化知识编译为可指导 Agent 执行的结构化任务生成模板。
...
#### 4.1 GraphNode（图谱节点 - 聚合根）
**定义**：从笔记中异步抽取出来的实体（如方法论概念、人物、理论），由 sqlite-vec 支持密集检索。
